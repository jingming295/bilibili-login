...
export class Insert extends CreateDatabase{
    ...
    async insertIntoBilibiliAccountData(SESSDATA:string, csrf:string, refresh_token:string, DedeUserID: string, DedeUserID__ckMd5: string, cookieData: MainPageCookie){
      const row = {
        id: 1,
        csrf: csrf,
